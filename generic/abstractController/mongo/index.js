...
let AbstractController = class AbstractController {
      ...
      switch (err.name) {
        case "ValidationError":
          return err.errors !== undefined
            ? this._mongoValidateErrorHandler(err)
            : this.errorMessages.databaseFailure;
          break;
        case "CastError":
          return this.errorMessages.typeMismatch;
          break;
        default:
          return this._mongoDatabaseErrorHandler(err);
          break;
      }
    } else {
  ...
  constructFindQuery(query) {
    ...
    var sort = {};
    var sortField = query.sortByAscending
    ...
    var findQuery = this.getFindQuery(
      filterQuery,
      query.fields,
      query.populate,
      query.limit,
      ...
      return this.mongoModel
        ...
        .then(result => {
          resolve({
            ...
            message: self.mongoSchema + " record created successfully"
          });
        })
        .catch(error => {
          reject({
      ...
      findQuery.exec((error, data) => {
        if (error) {
          ...
          reject({
            ...
            message: "No " + self.mongoSchema + " record found"
          });
        } else {
          resolve({
            data: data,
            status: httpStatus.ok,
            message: self.mongoSchema + "'s record found successfully"
          });
        }
      });
    });
      ...
      self.model.findOne(queryString).exec(function(error, data) {
        if (error) {
          ...
          resolve({
            data: data,
            status: httpStatus.ok,
            message: self.mongoSchema + " record found successfully"
          });
        }
      });
    });
    ...
    return new Promise((resolve, reject) => {
      if (self.validateObjectId(_id)) {
        ...
        reject({
          ...
          status: httpStatus.bad_request,
          message:
            "Invalid type of id has been sent for " + this.mongoSchema + " find"
        });
      }
    });
  }
    ...
    return new Promise((resolve, reject) => {
      if (self.validateObjectId(_id)) {
        self.model.findByIdAndUpdate(_id, update, options, (error, data) => {
          if (error) {
            ...
            resolve({
              data: data,
              status: httpStatus.ok,
              message: self.mongoSchema + " record updated successfully"
            });
          }
        });
      } else {
        reject({
          ...
          status: httpStatus.bad_request,
          message:
            "Invalid type of id has been sent for " + modelName + " update"
        });
      }
    });
  }
  ...
  _mongoFindOneAndUpdate(findQuery, update) {
    ...
    let options = { new: true, runValidators: true };
    return new Promise((resolve, reject) => {
      this.mongoModel.findOneAndUpdate(
        findQuery,
        update,
        options,
        (error, data) => {
    ...
    return new Promise((resolve, reject) => {
      self.model.remove(findQuery, (error, data) => {
        if (error) {
          ...
          resolve({
            status: httpStatus.ok,
            message: self.mongoSchema + " record deleted successfully"
          });
        }
      });
    });
