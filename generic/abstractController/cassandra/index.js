      ...
      model.save(err => {
        if (!err)
          resolve({
            ...
            message: self.cassandraSchema + " record created successfully"
          });
        else
          reject({
            error: err,
      ...
      self.cassandraModel.find({}, (error, data) => {
        if (error) {
          ...
          reject({
            ...
            message: "No " + self.cassandraSchema + " record found"
          });
        } else {
          resolve({
            data: data,
            status: httpStatus.ok,
            message: self.cassandraSchema + "'s record found successfully"
          });
        }
      });
    });
