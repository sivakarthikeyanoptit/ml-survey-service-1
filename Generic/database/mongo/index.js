...
module.exports = class database {
  constructor(defaults) {
    this.db_connection = mongoose.createConnection(
      defaults.connection.mongodb.host +
        "/" +
        defaults.connection.mongodb.database +
        (defaults.connection.mongodb.replicaSet != "" &&
        defaults.connection.mongodb.replicaSet != undefined
          ? "?replicaSet=" + defaults.connection.mongodb.replicaSet
          : ""),
      defaults.connection.mongodb
    );
  ...
  createModel(opts) {
    ...
    if (opts.plugins.autoPopulate) schema.plugin(autopopulate);
    //index
    // schema.index(opts.indices.fields || {}, opts.indices.options || {});

    var model = this.db_connection.model(opts.name, schema, opts.options);
    // custom methods
  ...
  objectExtend(default_obj, obj) {
    return merge.recursive(
      true,
      this.objectFinder(default_obj),
      this.objectFinder(obj)
    );
  }
};
